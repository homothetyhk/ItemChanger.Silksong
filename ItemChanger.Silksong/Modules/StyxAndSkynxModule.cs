...
public class StyxAndSkynxModule : Module, ICumulativeCostModule
{
    ...
    private void SpawnCocoonObjects(PlayMakerFSM fsm)
    {
        ...
        foreach (var (slot, cocoon) in Cocoons)
        {
            ...
            {
                var info = ContainerInfo.FromPlacement(placement, fsm.gameObject.scene, ContainerNames.SilkGrubCocoon, FlingType.Everywhere);
    ...
    private void ModifySkynxFsm(PlayMakerFSM fsm)
    {
        ...
        // Throwing items is triggered from a state that's on all dialogue pathways, so we record the player's responses on skynxInventory.
        fsm.MustGetState("Give Post").InsertMethod(0, _ => skynxInventory.WillTakeFree = true);
        ...
        var maybeCharityState = fsm.AddState("Maybe Charity?");
        maybeCharityState.AddTransition("TRUE", "Charity?");
        maybeCharityState.AddTransition("FALSE", "Deny");
        maybeCharityState.AddMethod(_ => fsm.SendEvent(skynxInventory.Free.Count > 0 ? "TRUE" : "FALSE"));

        giveGrubState.RemoveTransition("FALSE");
        giveGrubState.AddTransition("FALSE", "Maybe Charity?");
        ...
        {
            // Throwing items is triggered from a state that's on all dialogue pathways, so we record the player's responses on skynxInventory.
            skynxInventory.WillTakeNonFree = true;

            // Pay costs in reverse (largest first) so we only update the counter UI once.
        ...
        throwState.InsertMethod(0, _ =>
        {
            ...
            if (!skynxInventory.WillTakeAny()) fsm.SendEvent("FINISHED");
        });
        ...
        throwState.InsertMethod(
            ...
            _ =>
            {
                ...
                foreach (var item in skynxInventory.WillTake())
                {
    ...
    private record SkynxInventory
    {
        ...
        public bool WillTakeFree = false;
        public bool WillTakeNonFree = false;
        ...
        public bool WillTakeAny() => (Free.Count > 0 && WillTakeFree) || (CanBuy.Count > 0 && WillTakeNonFree);

        public IEnumerable<Item> WillTake()
        {
            if (WillTakeFree)
            {
