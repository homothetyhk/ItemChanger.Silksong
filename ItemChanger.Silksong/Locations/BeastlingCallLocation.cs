...
public class BeastlingCallLocation : AutoLocation
{
    ...
    private void HookBellEaterDefeat(PlayMakerFSM fsm)
    {
        ...
        startState.AddLambdaMethod(_ => fsm.SendEvent(InteractRequirements.Value ? "ENABLE INTERACT" : "FINISHED"));
        ...
        lTinkState.AddLambdaMethod(_ => fsm.SendEvent(InteractRequirements.Value ? "FINISHED" : "NO INTERACT"));

        FsmState rTinkState = fsm.MustGetState("Tink React R");
        rTinkState.AddTransition("NO INTERACT", "Idle R");
        rTinkState.AddLambdaMethod(_ => fsm.SendEvent(InteractRequirements.Value ? "FINISHED" : "NO INTERACT"));
