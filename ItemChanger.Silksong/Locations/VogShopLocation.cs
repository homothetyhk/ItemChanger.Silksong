...
{
    // Avoid scene data wonkiness by tracking this explicitly, globally.
    private bool TalkedToVog = false;
    ...
    private void ModifyDialogueFsm(PlayMakerFSM fsm)
    {
        ...
        metState.GetFirstActionOfType<BoolTest>()?.enabled = false;
        metState.AddMethod(_ =>
        {
            if (TalkedToVog) fsm.SendEvent("TRUE");
        });

        // Force the shop dialogue regardless of map inventory or flea collection state.
        ...
        fsm.MustGetState("Has Map?").InsertMethod(0, _ => fsm.SendEvent("TRUE"));
    }

    private static bool ShouldForceVogSpawn(CaravanTroupeLocations loc)
    {
    ...
    private void AddVogShopToFestivalFsm(PlayMakerFSM fsm)
    {
        ...
        var template = GameObjectKeys.VOG_SHOP().GetPrefab().GetComponent<CaravanTroupeHunter>();
