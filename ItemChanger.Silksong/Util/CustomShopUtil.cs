...
public static class CustomShopUtil
{
    ...
    {
        foreach (var shopDownEvent in ShopDownEvents())
            self.AddComponent<EventRegister>().SubscribedEvent = shopDownEvent.Name;

        var owner = self.AddComponent<ShopOwner>();
        var template = GameObjectKeys.SHOP().GetPrefab().GetComponent<ShopOwner>();
    ...
    /// </summary>
    public static FsmStateAction ShopUpAction(this ShopOwner self) => new ShopUpAction(self);
...
file class ShopUpAction(ShopOwner ShopOwner) : FsmStateAction
{
    ...
    {
        ShopOwner.ShopObject.LocateMyFSM("shop_control").SendEvent("SHOP UP");
    }
}
